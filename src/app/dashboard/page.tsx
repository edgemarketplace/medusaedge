...
function DashboardContent() {
  ...
  const previewUrl = searchParams.get("previewUrl") || `https://${subdomain}.edgemarketplacehub.com`
  const planType = (searchParams.get("planType") || "launch") as "launch" | "pro"
        ...
        return (
          <div className="space-y-6">
            <div className="flex items-center justify-between mb-8">
              ...
              <Button className="rounded-xl bg-slate-900 text-white">Export Orders</Button>
            </div>
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
               <div className="p-12 text-center">
                  ...
                  <p className="text-slate-500 text-sm mt-1 max-w-xs mx-auto">Your store is live! Once customers start buying, their orders will appear here.</p>
                  <Button variant="secondary" className="mt-6 rounded-xl" onClick={() => window.open(previewUrl, '_blank')}>Visit Storefront</Button>
               </div>
            </div>
          </div>
        )
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
               ...
               <p className="text-slate-500 font-medium">You don&apos;t have any customers yet.</p>
            </div>
          </div>
        )
      case "Settings":
          ...
          <div className="space-y-8">
            ...
            <div className="grid gap-8 max-w-3xl">
               <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
                  <h3 className="font-bold mb-6">General Information</h3>
                  <div className="space-y-4">
                     <div className="grid gap-2">
                        <label className="text-xs font-bold text-slate-500 uppercase">Business Name</label>
                        <Input defaultValue={businessName} className="rounded-xl bg-slate-50 border-none" />
                     </div>
                     <div className="grid gap-2">
                        <label className="text-xs font-bold text-slate-500 uppercase">Tagline</label>
                        <Input defaultValue={tagline} className="rounded-xl bg-slate-50 border-none" />
                     </div>
                  </div>
               </div>
               <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
                  <h3 className="font-bold mb-6 text-red-600">Danger Zone</h3>
                  <Button variant="secondary" className="text-red-600 border-red-100 hover:bg-red-50 rounded-xl">Deactivate Storefront</Button>
               </div>
            </div>
          </div>
        )
        ...
        return (
          <>
            ...
            <div className="mb-10 flex items-center justify-between">
              <div>
                 <h2 className="text-2xl font-bold text-slate-900">Welcome back, {businessName.split(' ')[0]}!</h2>
                 <p className="text-slate-500 mt-1">Your marketplace is live at <span className="text-blue-600 font-medium">{subdomain}.edgemarketplacehub.com</span></p>
              </div>
              <div className="flex gap-3">
                 <Link href="/build">
                    <Button className="rounded-xl flex items-center gap-2 bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-900/20">
      ...
      <aside className="w-64 bg-slate-900 flex flex-col shrink-0">
        <div className="p-6">
          ...
          <nav className="space-y-1">
            {[
              { name: "Overview", icon: <LayoutDashboard className="h-4 w-4" /> },
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors ${
                  activeTab === item.name ? "bg-slate-800 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800/50"
                }`}
              >
                {item.icon}
                {item.name}
        ...
        <div className="mt-auto p-6">
           <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
              ...
              <p className="text-white text-xs font-bold mb-1">{planType === 'pro' ? '⚡ Pro Scaling' : '🚀 Launch'}</p>
              <p className="text-[10px] text-slate-400 mb-3">{planType === 'pro' ? '1% transaction fee' : '5% transaction fee'}</p>
